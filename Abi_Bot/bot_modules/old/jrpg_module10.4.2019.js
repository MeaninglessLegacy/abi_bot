		...
		*/
		...
		for (var key in chr_stats){
			if(key == "max_hp"){
				...
				chr.combat_stats[key] = Math.floor(chr.attributes['stamina']*2.25 + chr.attributes['wisdom']*1.25)
			}else if(key == "faith"){
				chr.combat_stats[key] = Math.floor(chr.attributes['wisdom']*2.25 + chr.attributes['stamina']*1.25)
...
function formatt_stats(object){
	...
	*/
	...
	for(key in chr.passive_tags){
		fText = fText+ 
		chr.passive_tags[key].skill.skill_name+"\n"
	};
...
//function cast all status boosting skills
function cast_status(user){
	...
	if(Object.keys(chr.passive_tags).length > 0){
		for(key in chr.passive_tags){
			return_text += cast_status_skill(user, chr.passive_tags[key].skill);
...
function calc_damage(skill, user, target){
	...
	if(elemental_type != null){
		second_damage = first_damage-((target.jrpg_data.character.elemental_resistances[elemental_type]/100)*second_damage)
	};
...
function calc_heal(skill, user, target){
	...
	if(elemental_type != null){
		second_heal = total_power-((target.jrpg_data.character.elemental_resistances[elemental_type]/100)*total_power)
	};
...
function single_heal_handle(skill, user, target, weaken){
	...
	if(target_character.primary_stats.hp > 0){
		...
		if(Math.floor(Math.random() * Math.floor(100))<crit_chance){
			...
			var critical_damage = 125+user.jrpg_data.character.combat_stats.critical_damage-(target_eva/4);
				...
				if(key in skills_list){
					...
					*/
					if(passive.effect == "REDUCE_ALL_DAMAGE"){
						...
						//give return text
						return_text += "```css\n"+target_character.primary_stats.name+"'s ["+passive.skill_name+"] reduced damage by for "+mitigation+"!\n```"
					}
				}
			}
		};
								...
								if(logic == "self"){
									...
									if(this_battle.turn_order.length > 0){
										if(this_battle.turn_order[0] == author){
											...
											var guard_self = use_skill(users[author], users[author], guard, users);
											//remove user from list of turn order
											remove_from_array(this_battle.turn_order, author);
											//msg to server
											return guard_self
