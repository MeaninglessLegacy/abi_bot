...
class mob {
	constructor(id, mob_obj, tier){
		...
		this.mob_settings = mob_obj.mob_settings,
		...
		this.backup_stats = Object.assign({},mob_obj.combat_stats);
	...
	*/
	...
	for(var i = 0; i < stat_count; i++){
		...
		if(stat_chance > 0 && stat_chance <= obj.quality_settings.positive_chance){
			...
			//negative stat
			if(stat_chance > obj.quality_settings.positive_chance && stat_chance <= obj.quality_settings.positive_chance+obj.quality_settings.positive_chance){
			var new_stat = pick_stat(all_stats, obj.new_item[obj.stat_on_item]);
			if(new_stat != null){
	...
	if(stock_equipment != undefined && equipment_generation_settings != undefined){
		if(item_type in stock_equipment){
			...
			if(ability_count > 0){
				...
				for(key in stock_item.abilities.GUARANTEED){
				...
				for(var i = 0; i < guaranteed_abilities.length; i++){
					if(Object.keys(new_item.spell_enable).length < ability_count){
						...
						for(key in new_item.spell_enable){
...
function update_stats_mob(mob){
	...
	if(mob.hasOwnProperty('primary_stats')){
		...
		//reset mob stats
		mob.combat_stats = Object.assign({},mob.backup_stats);
	...
	if(mob.mob_settings.hasOwnProperty("mob_combat_ai")){
		...
		*/
		...
		if(mob_ai.type == "ROTATION"){
			...
			var current_action = (battle.turn - Math.floor((battle.turn-1)/rotation.length)*rotation.length)-1;
			//if exists
					...
					if(conditions.target == "enemy"){
						...
						for(var i = 0; i < all_parties.length; i++){
							...
							if(t_party != null){
								...
								for(var a = 0; a < add_targets.length; a++){
									...
									if(add_targets[a] in battle.threat_levels){
										if(battle.threat_levels[add_targets[a]].type == "player"){
											target_chr = users[add_targets[a]].jrpg_data.character;
										}else if(battle.threat_levels[add_targets[a]].type == "mob"){
						...
						if(targets_ids.length > 0){
							...
							for(var i = 0; i < targets_ids.length; i++){
								if(targets_ids[i] in battle.threat_levels){
									//add one member to start
									if(targets.length == 0){
										targets.push(battle.threat_levels[targets_ids[i]]);
									};
									//sort
									for(var a = 1; a < targets.length; a++){
										if(battle.threat_levels[targets_ids[i]].threat >= targets[a].threat){
											targets.splice(a,0,battle.threat_levels[targets_ids[i]]);
											break
										}else if(battle.threat_levels[targets_ids[i]].threat < targets[a].threat){
											targets.splice(a+1,0,battle.threat_levels[targets_ids[i]]);
											break
										}
									}
								}
							...
							if(conditions.target_condition == "highest_threat"){	
								...
								for(var i = 1; i < targets.length; i++){
									if(targets[i].threat == highest_threats[0]){
	...
	if(skills_list[skill.skill_name] != undefined){
		...
		*/
			...
			for(var i = 0; i < this_skill.target_stats.length; i++){
				//if the stat exists
				if(this_skill.target_stats[i] in chr.combat_stats){
					//base
					skill_power = Math.floor(this_skill.skill_power)
					//check type
					if(this_skill.bonus_effects.hasOwnProperty('effect_type')){
						if(this_skill.bonus_effects.effect_type == "FLAT"){
							...
							skill_power = Math.floor(chr.combat_stats[this_skill.target_stats[i]]*(this_skill.skill_power/100))
						};
					};
				};
				//floor
	...
	if(Object.keys(all_duels).length > 0){
		...
		for(key in all_duels){
			...
			if((tduel.duel_accepted == true)&&(tduel.duel_started == false)){
				...
				//generate threat_levels
				for(key in tduel.participants){
					tduel.threat_levels[tduel.participants[key].id] = {
						"target_name" : key,
						"id" : tduel.participants[key].id,
						"type" : tduel.participants[key].type,
						"threat" : 0
					}
				};
	...
	if(Object.keys(all_encounter_battles).length > 0){
		for(key in all_encounter_battles){
			...
			if((tencounter.combat_encounter_started == true)&&(tencounter.turn == 0)){
				...
				tencounter.turn += 1;
				//determine turn order
				var order = determine_turn_order([tencounter.TEAM_1, tencounter.TEAM_2], users);
				tencounter.turn_order = order[1];
				//set participants
				tencounter.participants = order[2];
				//generate threat_levels
				for(key in tencounter.participants){
					tencounter.threat_levels[tencounter.participants[key].id] = {
						"target_name" : key,
						"id" : tencounter.participants[key].id,
						"type" : tencounter.participants[key].type,
						"threat" : 0
					}
				};
				//return a message that the duel has begun
...
function single_hit_handle(skill, user, target, weaken){
	...
	if(Math.floor(Math.random() * 100)<hit_chance){
		...
		*/
		for(type in battles){
			for(key in battles[type]){
				for(p in battles[type][key].participants){
					if(battles[type][key].participants[p].id == user.id){
						battles[type][key].threat_levels[user.id].threat += Math.ceil(this_attack/100);
...
function use_skill(user, target, skill, users){
	...
	if(users[user] != undefined){
		user_character = users[user].jrpg_data.character
	}else if(active_mobs[user] != undefined){
		user_character = active_mobs[user]
	}
	var target_character = target;
	if(users[target] != undefined){
		target_character = users[target].jrpg_data.character
	}else if(active_mobs[target] != undefined){
		target_character = active_mobs[target]
	}
