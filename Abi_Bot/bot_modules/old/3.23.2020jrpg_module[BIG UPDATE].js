		...
		*/
		...
		for (var key in chr.combat_stats){
			if(key == "max_hp"){
				...
				chr.combat_stats[key] = Math.floor(chr.attributes['luck']*1.25 + chr.attributes['dexterity']*1.25)
			}else if(key == "critical_damage"){
				chr.combat_stats[key] = Math.floor(chr.attributes['strength']*1.3 + chr.attributes['intelligence']*1.3)
			}else if(key == "evasion"){
				chr.combat_stats[key] = Math.floor(chr.attributes['luck']*0.75 + chr.attributes['dexterity']*0.75)
			}else if(key == "hit"){
				chr.combat_stats[key] = Math.floor(chr.attributes['dexterity']*1.25+chr.attributes['strength']*0.05+chr.attributes['intelligence']*0.05)
			}
		};
							...
							for(var i = 0; i < targets_ids.length; i++){
								if(targets_ids[i] in battle.threat_levels){
									...
									for(var a = 0; a < targets.length; a++){
										if(battle.threat_levels[targets_ids[i]].threat >= targets[a].threat && i != 0){
											targets.splice(a,0,battle.threat_levels[targets_ids[i]]);
											break
										}else if(battle.threat_levels[targets_ids[i]].threat < targets[a].threat && i != 0){
