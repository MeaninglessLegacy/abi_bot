...
class mob {
	constructor(id, mob_obj, tier){
		...
		this.backup_elemental = Object.assign({},mob_obj.elemental_resistances);
		this.backup_status = Object.assign({},mob_obj.status_resistances);
...
function update_stats_mob(mob){
	...
	if(mob["primary_stats"] != undefined){
		...
		//reset all elemental and status resistances
		mob.elemental_resistances = Object.assign({},mob.backup_elemental);
		
		mob.status_resistances =Object.assign({},mob.backup_status);
...
function progress_battle(battle, users){
	...
	if(Object.keys(battle.participants).length > 0){
		for(key in battle.participants){
	...
	for(key in battle.participant_variables){
		...
		for(key in all_duels){
			...
			if((tduel.duel_accepted == true)&&(tduel.duel_started == false)){
				...
				for(key in tduel.participants){
					tduel.participant_variables[tduel.participants[key].id] = new participant_variables_obj(key, tduel.participants[key].id, tduel.participants[key].type);
				};
		...
		for(key in all_encounter_battles){
			...
			if((tencounter.combat_encounter_started == true)&&(tencounter.turn == 0)){
				...
				for(key in tencounter.participants){
					tencounter.participant_variables[tencounter.participants[key].id] = new participant_variables_obj(key, tencounter.participants[key].id, tencounter.participants[key].type);
				};
				//return a message that the duel has begun
	...
	if(debuff != undefined){
		...
		*/
		if(debuff.effect_name == "stat_down"){
			var power = 0;
			//find target stat
			if(debuff.effect in chr.combat_stats){
				var target_stat = chr.combat_stats[debuff.effect];
				if(debuff.potency_type == "FLAT"){
					power = debuff.power;
				}else if(debuff.potency_type == "PERCENT"){
					...
					if(key == "target_status" && attack_redirected == false){
						...
						if(Math.floor(Math.random()*100) < Beffect.proc_chance){
							...
							*/
							...
							return_text += cast_status_debuff(null, target_character, target_character.passive_tags[Beffect.name].debuff);
						};
					};
				};
			};
